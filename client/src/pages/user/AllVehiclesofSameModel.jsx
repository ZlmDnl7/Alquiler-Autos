    ...
    flex-wrap mt-10"
              ...
              {filterdData.map((cur) =>
                cur.isDeleted === "false" ? (
                  <div
                    ...
                    key={cur._id} // ✅ Corregido: Usando ID único en lugar del índice
                  >
                    <div className="mx-auto max-w-[320px] px-4 py-2 sm:px-6 sm:py-0 lg:max-w-7xl lg:px-8">
                      <div className="aspect-h-1 aspect-w-1 w-full overflow-hidden object-contain rounded-md bg-white lg:aspect-none group-hover:opacity-75 lg:h-80 mb-3">
                        <img
                          src={cur.image?.[0] || ''} // ✅ Verificación de seguridad
                          alt={cur.name} // ✅ Corregido: Removidas comillas innecesarias
                ...
                ) : null // ✅ Retornar null explícitamente para elementos filtrados
              )}
            </div>
          ) : (
            <div className="max-w-[400px] flex flex-col justify-center gap-y-4 items-center mx-auto mt-10">
              <img
                ...
                alt="No car found illustration"
              />
              <p className="text-md font-bold">No car found </p>
            </div>
          )}
          
          {!allVariants || (allVariants.length === 0 && <CarNotFound />)} {/* ✅ Corregido: === en lugar de == */}
        </div>
      </div>
    </>
  );
