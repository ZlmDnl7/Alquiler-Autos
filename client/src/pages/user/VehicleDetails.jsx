...
const VehicleDetails = () => {
  ...
  const getFuelTypeLabel = (fuelType) => {
    ...
    return fuelLabels[fuelType] || fuelType || "No especificado";
  };

  const handleImageNavigation = (direction) => {
    const mainImage = document.querySelector('.main-vehicle-image');
    if (!mainImage || !singleVehicleDetail?.image) return;

    const currentIndex = singleVehicleDetail.image.findIndex(img => img === mainImage.src);
    let newIndex;
    
    if (direction === 'prev') {
      newIndex = currentIndex > 0 ? currentIndex - 1 : singleVehicleDetail.image.length - 1;
    } else {
      newIndex = currentIndex < singleVehicleDetail.image.length - 1 ? currentIndex + 1 : 0;
    }
    
    mainImage.src = singleVehicleDetail.image[newIndex];
  };
  ...
  const hasMultipleImages = singleVehicleDetail.image?.length > 1;
                    ...
                    {singleVehicleDetail.succes !== false &&
                      singleVehicleDetail.image?.map((cur, idx) => (
                        <button
                          ...
                          key={`image-${idx}-${cur.substring(cur.lastIndexOf('/') + 1)}`}
                          onClick={() => changeMainImage(cur)}
            ...
            <div className="lg:col-span-2 lg:row-span-2 lg:row-end-2 bg-slate-50 px-10 min-h-screen pb-5 pt-10">
              ...
              <div className="mt-3 flex flex-col justify-center items-start select-none flex-wrap gap-4 font-mono text-[14px]">
                ...
                <div className={`${styles.iconFlex} capitalize border border-slate-200 p-2 rounded-md`}>
                  ...
                  Transmisión: {getTransmissionLabel(singleVehicleDetail.transmition)}
                </div>
                <div className={`${styles.iconFlex} capitalize border border-slate-200 p-2 rounded-md`}>
                  <span>
                    <FaCarSide />
                ...
                {singleVehicleDetail.car_description && (
                  <div className={`${styles.iconFlex} capitalize border border-slate-200 p-2 rounded-md col-span-2`}>
                    ...
                    <span>Descripción: {singleVehicleDetail.car_description}</span>
                  </div>
                )}
                ...
                <div className={`${styles.iconFlex} capitalize border border-slate-200 p-2 rounded-md`}>
                  ...
                  Tipo de Combustible: {getFuelTypeLabel(singleVehicleDetail.fuel_type)}
                </div>
                <div className={`${styles.iconFlex} capitalize border border-slate-200 p-2 rounded-md`}>
              ...
              <div className="mt-10 flex flex-col items-center justify-between space-y-4 border-t border-b py-4 sm:flex-row sm:space-y-0">
                ...
                <button
                  ...
                  onClick={() => handleBook(singleVehicleDetail._id)}
                >
                  <span>
                    <GrSecure />
                  </span>
                  <span>Reservar Auto</span>
                </button>
              </div>
            ...
            <div className="lg:col-span-3">
              <div className="border-b border-gray-300">
                <nav className="flex gap-4">
                  <button
                    type="button"
                    className="border-b-2 border-gray-900 py-4 text-sm font-medium text-gray-900 hover:border-gray-400 hover:text-gray-800 bg-transparent cursor-default"
                  >
                    Descripción
                  </button>
                </nav>
